...
for lidx, lab in enumerate(labs):
	...
	b = (behavior.TrialSet.Trial & (subject.SubjectLab() & 'lab_name="%s"'%lab)) \
		* (acquisition.Session() & 'session_start_time > "2019-05-01"'  & 'task_protocol LIKE "%biased%"') \
...
behav['lower_contrasts'] = behav.groupby(['subject_nickname', 'session_start_time', 
	'lab_name'])['signed_contrast'].transform(lambda x: 2 in x.unique())
...
fig.savefig(os.path.join(figpath, "contrast_sets_ines.pdf"))
print('done')
...
ax = sns.pointplot(x="lower_contrasts", y="bias_shift", dodge=True, join=True, data=biasshift, legend=False)
...
fig.savefig(os.path.join(figpath, "contrast_sets_stats_ines.pdf"))
