...
behav['session_day'] = behav.groupby(['lab_name', 'subject_nickname'])['session_date'].apply(lambda x: (x - np.min(x))).dt.days
...
# also remove animals with a lesion over V1
behav = behav[~behav['subject_nickname'].str.contains('CSK-les')]

lab_names = {'cortexlab':'Carandini & Harris lab', 'mainenlab':'Mainenlab', 'churchlandlab':'Churchland lab',
...
fig.savefig(os.path.join(figpath, "figure3a_learningcurves_perlab.pdf"))
fig.savefig(os.path.join(figpath, "figure3a_learningcurves_perlab.png"), dpi=600)
plt.close('all')
...
fig.despine(trim=True)
fig.savefig(os.path.join(figpath, "figure3a_learningcurves_permouse.pdf"))
plt.close('all')
